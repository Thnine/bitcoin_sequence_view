<template>
    <div style="position:relative">
        ...
        <el-select v-model="filterFlag" @change="handleFilterFlagChange" size="mini" style="position:absolute;left:10px;top:10px;width:120px;">
            <el-option :label="'全部'" :value="'all'"/>
            <el-option :label="'行为序列筛选'" :value="'filter'"/>
        </el-select>
        <el-radio-group v-model="mouseMode" @input="swithMouseMode()" size="mini" style="position:absolute;right:40px;top:10px;">
...
export default {
    ...
    methods:{
        start(nodes,links){//需要点数据nodes，格式为[{'id':'id1'},...]；需要边数据links，格式为[{'source':'id1','target':'id15','time':'2020-11-04 04:00'},...],
            const self = this;
